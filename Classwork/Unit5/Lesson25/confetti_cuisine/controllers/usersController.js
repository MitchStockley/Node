...
//Listing 25.8 Adding passport registration and flash messaging in the create action
  ...
  //adding the log in action to user controller. 
  login: (req, res) => {
    res.render("users/login"); //adds an action to render my form for browser viewing
  },
  //Listing 25.10 Adding a validate action in usersController.j
  validate: (req, res, next) => { //add validate action
    req
      .sanitizeBody("email")
      .normalizeEmail({
        all_lowercase: true
  ...
  authenticate: passport.authenticate("local", { //adding the authentication middleware with redirect and flash message options
